...
class PaperSearch:
    ...
    async def search_papers(
        ...
        # Search Semantic Scholar if enabled
        if (
            "semantic_scholar" in sources
            and ENABLE_SEMANTIC_SCHOLAR
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt > 0:
                    ...
                    logger.info(
                        f"Retrying download in {delay} seconds (attempt {attempt + 1}/{max_retries + 1})"
                    )
                ...
                if (
                    ...
                    and not paper_result.pdf_url.lower().endswith(".pdf")
                ):
                ...
                if content_length < 1024:  # Less than 1KB is suspicious
                    logger.warning(
                        f"Downloaded content is very small ({content_length} bytes), may not be a valid PDF"
                    )
                elif content_length > 50 * 1024 * 1024:  # More than 50MB is very large
                    logger.warning(
                        f"Downloaded content is very large ({content_length / 1024 / 1024:.1f} MB)"
                    )

                # Basic PDF validation - check for PDF header
                if response.content[:4] != b"%PDF":
                    logger.warning(
                        "Downloaded content does not appear to be a valid PDF (missing PDF header)"
                    )
                ...
                logger.info(
                    f"Successfully downloaded PDF ({content_length / 1024:.1f} KB)"
                )
                ...
                if attempt >= max_retries:
                    logger.error(
                        f"Failed to download PDF after {max_retries + 1} attempts due to timeout"
                    )
                    return None

            except httpx.HTTPStatusError as e:
                logger.warning(
                    f"HTTP error downloading PDF (attempt {attempt + 1}): {e.response.status_code} - {e}"
                )
                # Don't retry for certain status codes
                if e.response.status_code in [401, 403, 404, 410]:
                    logger.error(
                        f"Permanent error ({e.response.status_code}), not retrying"
                    )
                    return None
                if attempt >= max_retries:
                    logger.error(
                        f"Failed to download PDF after {max_retries + 1} attempts due to HTTP error"
                    )
                    return None

            except httpx.RequestError as e:
                logger.warning(
                    f"Request error downloading PDF (attempt {attempt + 1}): {e}"
                )
                if attempt >= max_retries:
                    logger.error(
                        f"Failed to download PDF after {max_retries + 1} attempts due to request error"
                    )
                    return None

            except Exception as e:
                logger.warning(
                    f"Unexpected error downloading PDF (attempt {attempt + 1}): {e}"
                )
                if attempt >= max_retries:
                    logger.error(
                        f"Failed to download PDF after {max_retries + 1} attempts due to unexpected error"
                    )
                    return None

        return None

    async def download_paper_from_url(
            ...
            try:
                ...
                if arxiv_match:
                    ...
                    if paper_result and paper_result.pdf_url:
                        ...
                        logger.warning(
                            f"Could not get arXiv paper info for ID: {arxiv_id}"
                        )
                        return None
            except Exception as e:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt > 0:
                    delay = 2**attempt  # 2, 4, 8 seconds
                    logger.info(
                        f"Retrying URL download in {delay} seconds (attempt {attempt + 1}/{max_retries + 1})"
                    )
                    await asyncio.sleep(delay)
                ...
                if "pdf" in content_type or url.lower().endswith(".pdf"):
                    ...
                    logger.info(
                        f"Successfully downloaded PDF from URL ({content_length / 1024:.1f} KB)"
                    )
                    return response.content
                else:
                ...
                logger.warning(
                    f"Timeout downloading from URL (attempt {attempt + 1}): {e}"
                )
                if attempt >= max_retries:
                    logger.error(
                        f"Failed to download from URL after {max_retries + 1} attempts due to timeout"
                    )
                    return None

            except httpx.HTTPStatusError as e:
                logger.warning(
                    f"HTTP error downloading from URL (attempt {attempt + 1}): {e.response.status_code} - {e}"
                )
                # Don't retry for certain status codes
                if e.response.status_code in [401, 403, 404, 410]:
                    logger.error(
                ...
                if attempt >= max_retries:
                    logger.error(
                        f"Failed to download from URL after {max_retries + 1} attempts due to HTTP error"
                    )
                    return None

            except httpx.RequestError as e:
                logger.warning(
                    f"Request error downloading from URL (attempt {attempt + 1}): {e}"
                )
                if attempt >= max_retries:
                    logger.error(
                        f"Failed to download from URL after {max_retries + 1} attempts due to request error"
                    )
                    return None

            except Exception as e:
                logger.warning(
                    f"Unexpected error downloading from URL (attempt {attempt + 1}): {e}"
                )
                if attempt >= max_retries:
                    logger.error(
                        f"Failed to download from URL after {max_retries + 1} attempts due to unexpected error"
                    )
                    return None
